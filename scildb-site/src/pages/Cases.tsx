    ...
    <div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <ChartCard
          ...
          subtitle="Grouped by the decade of each decision date (not the Court term), within the current filter — click a column to limit the year filter to that decade"
        >
          <DecadeColumns
            data={byDecade}
            series={[{ key: 'count', label: 'Cases', color: 'var(--viz-1)' }]}
            onClickDecade={(decade) => {
              const from = Math.max(data.yearMin, decade)
              const to = Math.min(data.yearMax, decade + 9)
              if (yearFrom === from && yearTo === to) {
                // clicking the already-selected decade clears it back to the full range
                setYearFrom(data.yearMin)
                setYearTo(data.yearMax)
              } else {
                setYearFrom(from)
