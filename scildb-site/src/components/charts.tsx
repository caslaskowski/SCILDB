...
export function DecadeColumns({
  ...
  return (
    <div style={{ height }} role="img" aria-label="Column chart of case counts by decade">
...
/* ── Diverging bars (support ↔ oppose), HTML marks ──────────────────────── */

export interface DivergingRow {
  key: string
  label: string
  sublabel?: string
  left: number // e.g. votes against the Native party's position
  right: number // e.g. votes for
}
    ...
    <div>
      <div className="mb-2 flex items-center justify-between text-xs text-ink3">
        <span className="flex items-center gap-1.5">
          <span aria-hidden="true" className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: 'var(--viz-neg)' }} />
          {leftLabel}
        </span>
        <span className="flex items-center gap-1.5">
          {rightLabel}
          <span aria-hidden="true" className="inline-block h-2.5 w-2.5 rounded-full" style={{ background: 'var(--viz-pos)' }} />
        </span>
      </div>
      <ul className="flex flex-col gap-1">
          ...
          const content = (
            <>
              <span className="flex w-full items-center gap-2">
                <span className="w-8 shrink-0 text-right text-xs text-ink2 tabular-nums">
                  {row.left > 0 ? formatNumber(row.left) : ''}
                </span>
                <span className="relative h-4 flex-1">
                  <span className="absolute inset-y-0 right-0 w-full overflow-hidden rounded-l-[4px]">
                    <span
                      className="absolute inset-y-0 right-0 rounded-l-[4px]"
                      style={{ width: `${(row.left / max) * 100}%`, background: 'var(--viz-neg)' }}
                    />
                  </span>
                </span>
                <span className="w-44 shrink-0 truncate text-center text-xs" title={row.label}>
                  <span className={selected ? 'font-semibold text-accent-strong' : 'font-medium text-ink'}>
                    {row.label}
                  </span>
                  {row.sublabel && <span className="ml-1 hidden text-ink3 sm:inline">{row.sublabel}</span>}
                </span>
                <span className="relative h-4 flex-1">
                  <span
                    className="absolute inset-y-0 left-0 rounded-r-[4px]"
                    style={{ width: `${(row.right / max) * 100}%`, background: 'var(--viz-pos)' }}
                  />
                </span>
                <span className="w-8 shrink-0 text-left text-xs text-ink2 tabular-nums">
                  {row.right > 0 ? formatNumber(row.right) : ''}
                </span>
              </span>
            </>
          )
            ...
            <li key={row.key}>
              {onClickRow ? (
                <button
                  ...
                  className={`w-full cursor-pointer rounded px-1 py-0.5 hover:bg-accent-wash ${selected ? 'bg-accent-wash' : ''}`}
                  title={`${row.label}: ${row.right} ${rightLabel.toLowerCase()}, ${row.left} ${leftLabel.toLowerCase()}`}
                >
                  {content}
                </button>
              ) : (
        ...
        {rows.length === 0 && <li className="py-4 text-center text-xs text-ink3">No data for this filter.</li>}
      </ul>
    </div>
  )
}
